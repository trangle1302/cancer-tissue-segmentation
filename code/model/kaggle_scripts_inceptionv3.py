...
class InceptionV3(nn.Module):
    def __init__(self, pretrained=True, num_channels = 3, aux_logits = True):
        ...
        self.bnclass_aux = nn.BatchNorm1d(1000)
        self.doclass_aux = nn.Dropout(0.5)
        self.dense_aux = nn.Linear(1000, 1)

        self.bnclass1 = nn.BatchNorm1d(1000)
        self.doclass1 = nn.Dropout(0.5)
        self.dense1 = nn.Linear(1000, 512)
        
        self.bnclass2 = nn.BatchNorm1d(512)
        self.doclass2 = nn.Dropout(0.5)
        self.dense2 = nn.Linear(512, 1)
        self.prob = nn.Softmax()
        
    def forward(self,x):
        if self.training and self.aux_logits:
            outs, aux_outs = self.inception(x)
            aux_outs = self.dense_aux(self.doclass_aux(self.bnclass_aux(outs)))
            outs = self.dense1(self.doclass1(self.bnclass1(outs)))
            outs = self.dense2(self.doclass2(self.bnclass2(outs)))
            outs = self.prob(outs)        
            return outs, aux_outs
        else:
            outs = self.inception(x)
            outs = self.dense1(self.doclass1(self.bnclass1(outs)))
            outs = self.dense2(self.doclass2(self.bnclass2(outs)))
            outs = self.prob(outs)    
            return outs
